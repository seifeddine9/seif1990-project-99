...
class Providers_Model extends CI_Model {
    ...
    public function insert($provider) {
     ...
     $provider['salt'] = generate_salt();
        $provider['password'] = hash_password($provider['salt'], $provider['password']);
    ...
    public function update($provider) {
        ...
        if (isset($provider['password'])) {
            $salt = $this->db->get_where('ea_users', array('id' => $provider['id']))->row()->salt;
            $provider['password'] = hash_password($salt, $provider['password']);
        }
    ...
    public function validate($provider) {
        ...
        /*if (isset($provider['username'])) {
            ...
            if (!$this->validate_username($provider['username'], $user_id)) {
        ...
        if (isset($provider['password'])) {
            if (strlen($provider['password']) < MIN_PASSWORD_LENGTH) {
