...
class Appointments_Model extends CI_Model {
    ...
    public function add($appointment) {
        ...
        //$this->validate_time_slot($appointment);
    ...
    public function exists($appointment) {
        if (!isset($appointment['start_datetime']) || !isset($appointment['end_datetime']) || !isset($appointment['id_users_provider']) || !isset($appointment['id_users_customer']) || !isset($appointment['id_services'])) {
    ...
    public function validate($appointment) {
        ...
        if ($appointment['is_unavailable'] == FALSE) {
            ...
            $num_rows = $this->db->get_where('ea_services', array('id' => $appointment['id_services']))->num_rows();
            if ($num_rows == 0) {
    ...
    /**
     * get filtred appointment number
     * */
    public function get_count_filter($date_debut, $date_fin) {
        ...
        $num_rows = $this->db
                        ...
                        ->where('ea_appointments.start_datetime <', $date_fin)
                        ->where('ea_appointments.start_datetime >', $date_debut)
                        ->get()->num_rows();
        ;
        return $num_rows;
    }
     ...
     * get appointment number
     * */
    public function get_count_price() {
        $num_rows = $this->db
     ...
     * get filtred appointment number
     * */
    public function get_count_filter_price($date_debut, $date_fin) {
    ...
    /**
     * get confirmed appointment number
     * */
    ...
    /**
     * get projected appointment number
     * */
    ...
    public function clear_google_sync_ids($provider_id) {
        ...
        $this->db->update('ea_appointments', array('id_google_calendar' => NULL), array('id_users_provider' => $provider_id));
    }

    public function validate_time_slot($appointment) {
        /**
          $num_rows = $this->db
          ->select('*')
          ->from('ea_appointments')
