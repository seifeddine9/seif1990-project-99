...
class User_Model extends CI_Model {
    ...
    public function save_settings($user) {
        ...
        if (isset($user['password'])) {
            ...
            $salt = $this->db->get_where('ea_users', array('id' => $user['id']))->row()->salt;
            $user['password'] = hash_password($salt, $user['password']);
        }
    ...
    public function get_salt($email) {
        $user =  $this->db->get_where('ea_users', array('email' => $email))->row_array();
    ...
    public function check_login($email, $password) {
        ...
        $user_data = $this->db
                ->select('ea_users.id AS user_id, ea_users.email AS user_email, '
                        . 'ea_roles.slug AS role_slug, ea_users.last_name ,'
                        . 'ea_users.first_name  , ea_users.phone_number')
                ->from('ea_users')
                ->join('ea_roles', 'ea_roles.id = ea_users.id_roles', 'inner')
                ->where('ea_users.email', $email)
                ->where('ea_users.password', $password)
                ->get()->row_array();
    ...
    public function regenerate_password( $email) {
        ...
        $salt = $this->db->get_where('ea_users', array('id' => $user_id))->row()->salt;
        $hash_password = hash_password($salt, $new_password);
        $this->db->update('ea_users', array('password' => $hash_password), array('id' => $user_id));
   ...
   /**
     * Retrieve user's avatar from database.
     *
     * @param string $username This will be used to find the user record.
     * @return string Returns the src avatar db value.
     */
    public function get_avatar($email) {
        $user =  $this->db->get_where('ea_users', array('email' => $email))->row_array();
         

        return  $user['src_photo'] ;
    }
