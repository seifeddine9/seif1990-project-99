...
class User extends CI_Controller {
  ...
  public function __construct() {
    ...
    $this->load->library( 'inc/facebook', $fb_config );
 ...
 $user_id = $this->session->userdata( 'user_id' );
  ...
  public function ajax_check_login_customer() {
    try {
      ...
      if ( $user_data ) {
        if ( $user_data['role_slug'] == "customer" ) {$this->session->set_userdata( $user_data ); // Save data on user's session.
          echo json_encode( $user_data );}
...
*/
  ...
  public function login_facebook() {
    ...
    $user_identified = $this->customers_model-> find_customer_id( $customer['idfacebook'] );

    if ( $user_identified ) {


      $this->session->set_userdata( 'user_id', $user_identified );
      $this->session->set_userdata( 'user_email', $customer['email'] );
      $this->session->set_userdata( 'role_slug', DB_SLUG_CUSTOMER );
      $this->session->set_userdata( 'first_name', $customer['first_name'] );
      $this->session->set_userdata( 'last_name', $customer['last_name'] );
      $customer_data = $this->customers_model-> get_row( $this->session->userdata( 'user_id' ) );
