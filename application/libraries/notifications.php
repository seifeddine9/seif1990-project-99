...
class Notifications {
    ...
    private function replace_template_variables($replace_array, $email_html) {
     ...
     /**
     * Send an email with the waiting details.
     *
     ...
     */
    public function send_waiting_details($appointment_data, $provider_data, $service_data,
        ...
        $email_html = file_get_contents(dirname(dirname(__FILE__))
                . '/views/emails/waiting_details.php');
...
//Set this to true if SMTP host requires authentication to send email
        $smtpuserid = "jandoubiseif.cj@gmail.com";                 
        $smtppassword = "seifeddine9";                                     
        $mail->IsHTML(true);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = $title;
        ...
        $mail->Host = $smtphost;
        $mail->Port = $smtpport;
        $mail->Username = $smtpuserid; // SMTP username
...
//Set this to true if SMTP host requires authentication to send email
    ...
    public function new_account($password, $email , $last_name , $first_name ,$company_settings) {
        $replace_array = array(
            '$email_title' => $this->ci->lang->line('Hello_message'),
            '$email_message' => $this->ci->lang->line('welcome_message'),
            '$company_name' => $company_settings['company_name'],
            '$company_email' => $company_settings['company_email'],
            '$company_link' => $company_settings['company_link'],
            '$password' => '<strong>' . $password . '</strong>',
            '$email' => '<strong>' . $email . '</strong>',
            '$last_name' => '<strong>' . ucfirst($last_name) . '</strong>',
            '$first_name' => '<strong>' . ucfirst($first_name) . '</strong>'
       ...
       $mail->AddEmbeddedImage("assets/img/logo_soc.png", "my-attach", "rocks.png");
        $mail->IsHTML(true);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = $this->ci->lang->line('new_account');
        $mail->Body = $email_html;
         $smtphost = "ssl://smtp.gmail.com";
        $smtpport = 465;  
//Set this to true if SMTP host requires authentication to send email
       ...
       public function new_cv( $email , $last_name , $first_name , $mobile  , $age , $etat_civil , $addresse , $city , $company_settings , $full_path , $service ) {
        if($service == "")
        {
            $service ="--";
        }
        if ($etat_civil == "no option")
        {
           $etat_civil = "--" ;
        }
        $replace_array = array(
            '$email_title' => 'Nouvelle Condidature',
            '$email_message' => 'Nouvelle Condidature',
            '$company_name' => $company_settings['company_name'],
            '$company_email' => $company_settings['company_email'],
            '$company_link' => $company_settings['company_link'],
            '$email' => '<strong>' . $email . '</strong>',
            ...
            '$first_name' => '<strong>' . ucfirst($first_name) . '</strong>',
            '$etat_civil' => '<strong>' . $etat_civil . '</strong>',
            '$addresse' => '<strong>' . $addresse . '</strong>',
        ...
        $mail->From = $company_settings['company_email'];
        $mail->FromName = $company_settings['company_name'];
        $mail->AddAddress($company_settings['company_email']); // "Name" argument crushes the phpmailer class.
           $mail->AddEmbeddedImage("assets/img/logo_soc.png", "my-attach", "rocks.png");
        $mail->IsHTML(true);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = 'Nouvelle Condidature reçu';
...
//Set this to true if SMTP host requires authentication to send email
       ...
       public function new_message($contactName, $contactAddress, $contactMessage , $contactSubject , $company_settings) {

        if($contactMessage == "")
        {
            $contactMessage ="Aucune Description";
        }
        $replace_array = array(
            '$email_title' => 'Nouveau Meesage',
            '$email_message' => 'Nouveau Meesage',
            '$company_name' => $company_settings['company_name'],
            '$company_email' => $company_settings['company_email'],
            '$company_link' => $company_settings['company_link'],
            '$contactName' => '<strong>' . $contactName . '</strong>',
            '$contactAddress' => '<strong>' . $contactAddress . '</strong>',
            '$contactMessage' => '<strong>' . $contactMessage . '</strong>',
            '$contactSubject' => '<strong>' . $contactSubject . '</strong>'

        );

        $email_html = file_get_contents(dirname(dirname(__FILE__))
                    . '/views/emails/new_message.php');
        $email_html = $this->replace_template_variables($replace_array, $email_html);
        ...
        $mail->IsHTML(true);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = 'Nouveau Meesage reçu';
        $mail->addAttachment($full_path);
